...
public class RecordService {
    ...
    public String convertStadium(String s) {
        ...
        return switch(s) {
            // 1. 잠실 야구장
            case "잠실", "잠실 야구장" -> "잠실 야구장";

            // 2. 사직 야구장
            case "사직", "사직 야구장" -> "사직 야구장";

            // 3. 대구삼성라이온즈파크
            case "대구", "대구삼성라이온즈파크" -> "대구삼성라이온즈파크";

            // 4. 고척 SKYDOME
            case "고척", "고척 SKYDOME" -> "고척 SKYDOME";

            // 5. 한화생명 볼파크
            case "대전(신)", "한화생명 볼파크", "한화생명 이글스 파크", "대전한화생명이글스파크" -> "한화생명 볼파크";

            // 6. 기아 챔피언스 필드
            case "광주", "기아 챔피언스 필드", "광주-기아챔피언스필드" -> "기아 챔피언스 필드";

            // 7. 수원 케이티 위즈 파크
            case "수원", "수원 케이티 위즈 파크", "KT 위즈 파크" -> "수원 케이티 위즈 파크";

            // 8. 창원 NC 파크
            case "창원", "창원 NC 파크", "NC 파크" -> "창원 NC 파크";

            // 9. 인천 SSG 랜더스필드 (요청하신 "랜더스필드"는 붙여씁니다)
            case "문학", "인천 SSG 랜더스필드", "SSG 랜더스 필드" -> "인천 SSG 랜더스필드";

            // 그 외 알 수 없는 값은 그대로 반환 (추후 케이스 추가)
            default -> s;
        };
    }
